...
class CustomBatchNormManualFunction(torch.autograd.Function):
  ...
  def forward(ctx, input, gamma, beta, eps=1e-5):
    ...
    norm_factor = torch.rsqrt(var + eps)

    # Compute backward pass tensors
    x_hat = (input - mu) * norm_factor
    norm_backward = gamma * norm_factor

    out = gamma*x_hat + beta

    # Store backward pass data
    ctx.save_for_backward(x_hat,norm_backward)
    ctx.eps = eps
  ...
  def backward(ctx, grad_output):
    ...
    x_hat, norm_backward = ctx.saved_tensors

    # Compute parameter gradients
    if ctx.needs_input_grad[0] or ctx.needs_input_grad[2]:
        grad_beta = torch.sum(grad_output,dim=0)
        #grad_beta = torch.einsum("sj->j",(grad_output,))
    else:
        grad_beta = None

    if ctx.needs_input_grad[0] or cts.needs_input_grad[1]:
        grad_gamma = torch.sum(x_hat*grad_output,dim=0)
        #grad_gamma = torch.einsum("sj,sj->j",(x_hat,grad_output))
    else:
        grad_gamma = None

    # Compute input gradient for next backward pass
    if ctx.needs_input_grad[0]:
        grad_input = norm_backward * (grad_output - (grad_beta + x_hat*grad_gamma) / batch_size)
    else:
        grad_input = None
...
class CustomBatchNormManualModule(nn.Module):
  ...
  def forward(self, input):
    ...
    mybatchnorm = CustomBatchNormManualFunction()
    out = mybatchnorm.apply(input,self.gamma,self.beta,self.eps)
